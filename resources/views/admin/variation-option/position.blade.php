...
@section('page-title', 'Variation position')
                    ...
                    <div class="card-header">
                        <div class="row mb-3">
                            <div class="col-md-12 text-right">
                                <a href="{{ route('admin.product.variation.create') }}" class="btn btn-sm btn-primary"> <i class="fa fa-plus"></i> Create</a>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <ul class="nav nav-underline">
                                    <li class="nav-item">
                                        <a class="nav-link" aria-current="page" href="{{ route('admin.product.variation.list') }}">List</a>
                                    </li>
                                    <li class="nav-item">
                            ...
                            <div class="col-md-6">
                                ...
                                <p class="small text-muted text-right mb-0">Drag &amp; drop table content to re-order their position</p>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <table class="table table-sm table-hover mb-3">
                            ...
                            <tbody class="sortable" data-position-update-route="{{ route('admin.product.variation.position.update') }}" data-csrf-token="{{ csrf_token() }}">
                                @forelse ($data as $index => $item)
                                    <tr class="single" id="{{ $item->id }}">
                                        <td>{{ $index + $data->firstItem() }}</td>
                                        <td>
                                            <p class="@if (count($item->activeOptions) == 0) text-danger fw-bold @endif">{{ $item->title }}</p>
                                        </td>
                                        <td>
                                            <p class="mb-0 text-muted">{{ count($item->options) }}</p>

                                            <p class="small mb-1 text-dark fw-bold">
                                            @foreach($item->activeOptionsLimited as $option)
                                                {{ $option->value }}@if(!$loop->last), @endif
                                            @endforeach
                                            @if (count($item->activeOptions) > 3)...@endif
                                            </p>
                                        </td>
                                        <td>
